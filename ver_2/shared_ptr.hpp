...
template <
  ...
  typename Allocator,
  // The deletor for the control object
  typename Deletor,
  ...
  : control_block_base<T>
{
   ...
   * Constructor.
   */
  control_block(Allocator&& allocator, Deletor&& deletor)
    : control_block_base<T>()
    , alloc_(std::forward<Allocator>(allocator))
    , deletor_(std::forward<Deletor>(deletor))
  {
  }
...
public:
  ...
  explicit shared_ptr(Args&&... args)
    : shared_ptr(Allocator{}, Deletor{}, std::forward<Args>(args)...)
  {
    //....
  }
...
{
  ctrl_blk_ = detail::control_block_base<T>::template create<Allocator, Deletor, RCP>(
...
template <typename... Args, typename Allocator,
          typename Deletor, typename Cond>
